...
function vyberZastavku(index: number): void {
    ...
    try{
        ...
        syncDisplayState();
    }catch(e){console.warn('Nelze aktualizovat displej:', e);}
...
function setupLogoutAndClock(): void {
    ...
    if (odhl) {
        odhl.addEventListener('click', function(ev){
            ev.preventDefault();
            try{ sessionStorage.clear(); localStorage.removeItem('prihlasenRidic'); } catch(e){}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if(openBtn){ openBtn.addEventListener('click', ()=> window.open('displej-cestujici.html','_blank')); }
...
function syncDisplayState(): void {
    ...
    }catch(e){ console.warn('Chyba pri exportu stavu displeje:', e); }
}