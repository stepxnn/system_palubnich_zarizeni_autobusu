...
// Tvar stavu, který panel řidiče ukládá do localStorage pod klíčem 'displejState'.
interface StavDispleje {
    cisloLinky: number;
    aktualniZastavka: string;
    seznamZastavek: string[];
    aktualniCas: string;        // datum uložené jako ISO text (localStorage umí jen text)
    predpokladanyCas: string;
    aktualniPasmo?: string;
    seznamPasem?: string[];
}

// Pomocná funkce: bezpečně nastaví text elementu podle jeho ID.
// Pokud hodnota chybí, zobrazí se pomlčka.
function setText(id: string, value: string | undefined | null): void {
    const el = document.getElementById(id);
    if (el) el.textContent = value ?? '—';
}

// Pomocná funkce: převede datum na český čas ve formátu HH:MM.
function formatujCas(datum: Date): string {
...
function renderState(state: StavDispleje | null): void {
    if (!state) return;
    ...
    if (list) {
        ...
        slice.forEach((s: string) => {
            const li = document.createElement('li');
            li.textContent = s;
            // Aktuální (příští) zastávka je zvýrazněná tučně.
            if (s === state.aktualniZastavka) li.style.fontWeight = 'bold';
            list.appendChild(li);
        });
    ...
    if (pasList) {
        pasList.innerHTML = '';
        (state.seznamPasem || []).forEach((p: string) => {
            const li = document.createElement('li');
            li.textContent = p;
...
function nactiStav(raw: string): void {
    try {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    window.addEventListener('storage', (ev) => {
        if (ev.key === 'displejState' && ev.newValue) nactiStav(ev.newValue);
    });
...
// Prázdný export říká TypeScriptu, že jde o modul (proměnné nejsou globální).
export {};
