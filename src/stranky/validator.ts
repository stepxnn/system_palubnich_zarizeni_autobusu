...
function updateHeaderTime(): void {
...
function updateTicketInfo(): void {
    try {
        ...
        if (ticket.doplnky && ticket.doplnky.length > 0) {
            ...
            if (addonsContainer) addonsContainer.style.display = 'block';
...
function alignAndSchedule(): void {
    const now = new Date();
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    window.addEventListener('storage', (ev) => {
        if (ev.key === 'displejState') updateStopInfo();
        if (ev.key === 'currentTicket') updateTicketInfo();
    });
