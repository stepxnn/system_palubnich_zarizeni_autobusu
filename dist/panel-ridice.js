...
function zvyrazniZastavku(index) {
...
function nastavTextyVyberu(od, kam, pasma) {
    ...
    if (odEl)
        odEl.textContent = od;
    if (kamEl)
        kamEl.textContent = kam;
    if (pasemEl)
        pasemEl.textContent = String(pasma);
}
// Přepnutí barvy tlačítka: zelená = vybrané, modrá = nevybrané.
function obarviTlacitko(btn, vybrane) {
...
function dalsiZastavka() {
    // Na poslední zastávce už není kam jet.
    if (aktualniIndexZastavky >= linka.zastavky.length - 1)
        return;
...
function setupNavigaci() {
    ...
    if (openDisplejBtn)
        openDisplejBtn.onclick = () => window.open('displej-cestujici.html', '_blank');
    const validatorBtn = el('open-validator-btn');
    if (validatorBtn)
        validatorBtn.onclick = () => window.open('validator.html', '_blank');
    // Dispečink se otevírá ve stejném okně (řidič se pak vrací tlačítkem Zpět).
    const dispecBtn = el('dispecink-btn');
    if (dispecBtn)
        dispecBtn.onclick = () => { window.location.href = 'dispecer-komunikace.html'; };
}
// ------------------------------------------------------------
// PRODEJ JÍZDENEK
// ------------------------------------------------------------
function setupTickets() {
    ...
    let zastavkaOd = null; // index výchozí zastávky
    let zastavkaDo = null; // index cílové zastávky
    let pocetPasem = 0; // přes kolik tarifních pásem se jede
    let vybranyTypJizdneho = null; // zvolené tlačítko typu jízdného
    let vybraneDoplnky = []; // zvolená tlačítka doplňků (kolo, kufr...)
    const typyTlacitka = document.querySelectorAll('.ticket-type-btn');
    const doplnkyTlacitka = document.querySelectorAll('.ticket-addon-btn');
    ...
    function zobrazVybranyTah() {
        const tah = el('vybrany-tah');
        if (tah)
            tah.style.display = (zastavkaOd !== null && zastavkaDo !== null) ? 'block' : 'none';
        const list = el('zastavkovy-list');
        if (!list)
            return;
        Array.from(list.querySelectorAll('.stop-item')).forEach((btn, index) => {
            btn.classList.remove('w3-blue', 'w3-light-green', 'w3-pale-green', 'active', 'stop-od', 'stop-kam');
            if (index === zastavkaOd)
                btn.classList.add('stop-od'); // výchozí = modrá
            else if (index === zastavkaDo)
                btn.classList.add('stop-kam'); // cílová = zelená
        });
    }
    ...
    if (zastavkovyList) {
        ...
        zastavkovyList.addEventListener('click', (e) => {
            var _a;
            const button = e.target.closest('.stop-item');
            if (!button)
                return;
            const buttons = Array.from(zastavkovyList.querySelectorAll('.stop-item'));
            const index = buttons.indexOf(button);
            if (index === -1)
                return;
            const nazev = ((_a = button.textContent) === null || _a === void 0 ? void 0 : _a.trim()) || '';
            if (zastavkaOd === null) {
    ...
    if (vydatBtn) {
        vydatBtn.onclick = () => {
            ...
            if (vybranyTypJizdneho) {
                const koeficient = (_a = koeficientyTypu[vybranyTypJizdneho.id]) !== null && _a !== void 0 ? _a : 1.0;
            ...
            const zastavkaOdNazev = ((_b = zastavky[zastavkaOd]) === null || _b === void 0 ? void 0 : _b.nazev) || '?';
            const zastavkaDoNazev = ((_c = zastavky[zastavkaDo]) === null || _c === void 0 ? void 0 : _c.nazev) || '?';
